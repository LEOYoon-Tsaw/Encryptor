...
func enigmaMachine(mappings: Configuration) -> EnigmaTranslator {
    ...
    func rotorCreator(mapping: [Character: Character], frequency: Int) -> (EnigmaTranslator, EnigmaTranslator) {
        ...
        func rotor(input: Character) throws -> Character {
            if turn % UInt(frequency) == 0 {
                let first = leftSide.removeFirst()
                leftSide.append(first)
            }
            guard let index = leftSide.indexOf(input) else {
                throw Errors.invalidCharacters(character: input)
            }
            extraTurn = extraTurn &+ UInt(index % 3)
            return rightSide[index]
        }
        func reverseRotor(input: Character) throws -> Character {
            guard let index = rightSide.indexOf(input) else {
                throw Errors.invalidCharacters(character: input)
            }
            extraTurn = extraTurn &+ UInt(index % 3)
    ...
    func reflectorCreator(mapping: [Character: Character]) -> EnigmaTranslator {
        ...
        func reflector(input: Character) throws -> Character {
    ...
    var encoders = [reflectorCreator(mappings.1)]
    for (connections, frequency) in mappings.0.reverse() {
        let (frontRotor, reverseRotor) = rotorCreator(connections, frequency: frequency)
        encoders.insert(frontRotor, atIndex: 0)
        encoders.append(reverseRotor)
    }

    func encode(input: Character) throws -> Character {
        var character = input
